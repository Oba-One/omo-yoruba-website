...
sit in a single flat `packages/` directory with `@oy/` names (`@oy/web`, `@oy/storybook`,
`@oy/ui`, `@oy/tokens`, `@oy/content`, `@oy/lint`), the layout of the owner's green-goods
...
- A deployable package carries its own `vercel.json` (Root Directory `packages/web`).
