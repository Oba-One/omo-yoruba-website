...
| `PathRow` | the row form of a door: chip by way in (Membership and Partnership from the homepage rows, Volunteer and Give from the Lessons page's take-part rows), title, line, action, the prototype's accent per door; stacked by `PathRows` (`content/`) with the label style | `door`, `primary`; `PathRows`: `labels` | Home, Get Involved |
