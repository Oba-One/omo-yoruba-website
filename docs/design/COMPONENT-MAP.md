...
| `GlanceStrip` | 4 and 5 facts, each value or its chip, a note or the note's chip; the caption under the band; the Pending line in the band with no facts; without its band inside a column (an initiative's four facts), each cell drawn by `GlanceCell` | `facts[]`, `caption`, `band`, `pending`, `edit`, `id` | Odunde, Gala, Lessons, Collective |
...
| `PathRow` | the row form of a door (chip by door key: Membership, Partnership, Volunteer, Give) or of a take-part row (its own way in, chip, title, line and action); the accent per way in; outline, gold once per view or quiet (`variant`); the registry's chip where a row misses its title or button; the default slot continues the line; stacked by `PathRows` (`content/`) with the label style | `door` or `way`, `chip`, `title`, `line`, `action`, `pending`; `primary`, `variant`, `edit`; `PathRows`: `labels` | Home, Odunde, Gala, Programs, Lessons, Collective, Get Involved |
...
| `PersonCard` | with portrait; without (woven tick instead of a face, a real design); compact; the role as a kicker, the name as a heading or its chip, the bio or, where the page asks for one, its chip | `person`, `variant`, `namePending`, `bioPending`, `edit` | Gala honorees, Lessons (then Our Story) |
...
| `FactList` | the plan-your-visit description list, two columns or one (`columns={1}`, a card or a narrow split), each value or its chip, a value a link only through `safeHref` with a 44px target; the Pending line with no facts | `facts[]`, `pending`, `columns` | Odunde, Programs, `ContactBlock` |
| `ListRow` | sponsor tier (Phase 5: name, amount, the ticked recognition, chips for what is owed, listed by `SponsorLevels` with the Pending line); `entry` (a title and its line in one column, listed by `EntryList` with the Pending line: the Lessons levels); `event` (the month and day, the title, the summary, the weekday and time with the venue or its chip, a quiet action; the page words the date in Los Angeles time, listed by `EventList` with the Pending line: the Collective's events); news entry with its page later | `row`, `kind`, pending wordings, `action`; `SponsorLevels`: `levels[]`; `EntryList`: `entries[]`, `pending`, `linePending`; `EventList`: `events[]`, `action`, `pending`, `venuePending` | Gala, Lessons, Collective (then News, Get Involved) |
...
| `EnquiryCard` | the card that explains a form before opening it: title, what it asks, how many questions, what happens next (from the page), the trigger as a link to `?enquiry=<kind>#enquiry`; the sentence "Or email" with an address (`mailto:` through `safeHref`) or its chip | `kind`, `title`, `blurb`, `next`, `label`, `variant`, `email`, `emailPending` | Get Involved doors, Lessons, Odunde, Gala |
