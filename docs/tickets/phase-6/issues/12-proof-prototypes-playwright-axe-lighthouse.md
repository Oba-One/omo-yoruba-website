...
- [ ] Playwright and axe on the three routes, seeded and with the placeholder project (`--workers=1`);
...
- [ ] The routes in `lighthouserc.cjs`; lhci on the local production build, mobile and desktop, numbers
      recorded here
- [ ] `bun check`, `bun run build` and the Storybook build (play functions finishing in the canvas)
