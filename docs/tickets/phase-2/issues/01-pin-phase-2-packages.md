...
- [x] Every new dependency matches the pins in wayfinder ticket 14 and `docs/research/phase-2-*.md`
- [x] `bun install` reports no peer warnings for the Sanity set; `bun run typecheck` passes
- [x] `packages/content/sanity.config.ts` exports the Studio config; `packages/web/sanity.config.ts`
