...
- Type at 0.24.0: `BlueprintFunctionResourceEventName = 'publish' | 'create' | 'delete' | 'update'`;
  ...
  --help`, `sanity functions test --help`.
...
- Handler reference: the module must export `handler`; `token` is "A token with access to your
  Sanity project" (obfuscated in logs); `local` is `undefined` in production. Deploy fails unless
  the built entry exports `handler`; the entry is `package.json#main`, then `index.ts`, then
  `index.js` inside `src`. Sources: https://www.sanity.io/docs/specifications/function-wrapper,
...
- A custom robot token (`robotToken: '$.resources.editor-robot.token'`, `defineRobotToken`) lets
  ...
  permissions". So the default token can read `siteSettings` and patch `enquiry`. Locally
  `clientOptions` "only provides projectId and apiHost unless you pass additional flags":
  `--dataset` adds the dataset, `--with-user-token` "Prime access token from CLI config". Sources:
  https://www.sanity.io/docs/blueprints/blueprints-robot-tokens,
...
- Delivery semantics are undocumented: the introduction page contains none of the words retry,
  ...
  or times out. Unverified; the documented retry policy belongs to webhooks. Sources:
  https://www.sanity.io/docs/functions/functions-introduction,
...
- "Functions run on Node.js v24.x"; timeout default 10 s, 1 to 900 s; memory default 1 GB, up to 10
  ...
  function count against the project API quota. Region and cold start are not documented anywhere
  read (unverified). Source: https://www.sanity.io/docs/functions/functions-introduction.
- `runtime?: 'node' | 'nodejs22.x' | 'nodejs24.x'`, default `nodejs24.x`, exists in the types but
  not on the reference page as read; whether the platform honours `nodejs22.x` is unverified.
  Source: `node_modules/@sanity/blueprints/dist/types/functions/index.d.ts`.
...
- The CLI reads no `.env` file (no dotenv anywhere in `@sanity/runtime-cli/dist`); a local test
  spawns `node` with `{...process.env, ...resource.env}`; Bun itself loads `.env`, `.env.local` and
  the `NODE_ENV` variants for `bun run` scripts, so a `packages/content` script that calls `sanity
  functions test` inherits `RESEND_API_KEY` from `packages/content/.env` (the runbook's location);
  `bunx` is not covered by that page (unverified). Sources:
...
- `functions test [NAME]`: `-d/--data` inline JSON, `-f/--file`, `--document-id` with `--dataset`
  and `--project-id`, `-e/--event create|update|delete`, `--data-before/--data-after`,
  `--document-id-before/-after`, `--file-before/-after` for update deltas, `-t/--timeout`,
  `-a/--api`, `--with-user-token`, `--no-wait`, `--json`. `functions dev` is a browser emulator on
  `localhost:8080` (`--port`, `--host`, `--timeout`). Both set `context.local`. `functions add
  --installer` offers `skip|npm|pnpm|yarn` only: use `--installer skip` and `bun add`. Sources:
  `sanity functions test|dev|add --help`,
...
- `blueprints init` writes the manifest plus `.sanity/blueprint.config.json` (scope and stack ids,
  ...
  `-m`); CI: `blueprints mint-deploy-token --print`, then `SANITY_AUTH_TOKEN`, `SANITY_PROJECT_ID`
  (or `SANITY_ORGANIZATION_ID`) and `SANITY_BLUEPRINT_STACK_ID`; ids resolve flags > env > module >
  config. Sources: `sanity blueprints init|plan|deploy --help`,
...
- Manifest discovery: names `blueprint.{json,js,mjs,ts}` and `sanity.blueprint.{json,js,mjs,ts}`,
  ...
  `.sanity/blueprint.config.json` is read from the manifest's directory; `src` resolves relative to
  the manifest, defaulting to `functions/<name>`. `sanity.cli.ts` is never read; its presence only
  triggers "Blueprint should not be co-located with a Sanity Studio". Sources:
...
- Dependencies live in a `package.json` next to the manifest (project level) or inside the function
  directory (function level); "Functions will not use or mix both sources"; "The CLI hydrates each
  Function's dependencies from that root install and packages them into an asset before uploading";
  TypeScript in pnpm workspaces "bundles inline using Vite"; otherwise "the CLI externalizes
  dependencies and ships them as a node_modules folder alongside the source"; assets cap at 200 MB;
  native modules are rejected. Sources: https://www.sanity.io/docs/functions/function-dependencies,
...
- The code path (`prepareAsset`): a `.ts` entry (or `transpile: true`) runs `vite.build` (Rolldown,
  ...
  `<function>/.build/function-<name>/` with a copied `package.json`; then, unless bundled,
  `@architect/hydrate` installs dependencies (npm unless `pnpm-lock.yaml` or `yarn.lock` sits next
  to the manifest; `bun.lock` means npm); native modules and folders over 200 MB fail; the folder is
  zipped and uploaded. Sources:
...
- Workspace imports: nothing documents a Bun workspace. A bare `import '@oy/lint'` resolves through
  the symlink Bun places in `node_modules`; whether Rolldown externalises it or emits it is
  unverified. A relative import or a tsconfig `paths` alias into `packages/lint/src` (and its JSON)
  goes through the same transpile, but emission of files above `preserveModulesRoot` is unverified.
  npm cannot install a `workspace:*` specifier, so never list `@oy/lint` in a function
  `package.json`. `sanity functions test` runs the identical `transpileFunction` and leaves
  `.build/function-<name>` on disk, so the prototype can inspect the output before the first deploy.
  Sources: `.../utils/functions/invoke/prepare.js`, `.../utils/functions/transpile/function.js`.
...
- Option 2, write a separate report document: the recycling bin guide's function does
  ...
  `S.documentList().title('...').filter('_type == "lintReport" && count(findings) > 0')`. Sources:
...
- `enquiry-notify`: `defineDocumentFunction({name: 'enquiry-notify', event: {on: ['create'], filter:
  ...
  owner's open decision (wayfinder ticket 02); with retries undocumented, the idempotency key, the
  filter and the `ifRevisionId` patch are the defence. Sources: the sections above.
- `content-lint`: `on: ['create', 'update']`, filter listing the content types, reads
  ...
  read `.build` before the first deploy to settle whether the workspace code is emitted. Sources:
  the bundling section above, `docs/adr/0010-voice-lints-from-one-word-list.md`.
...
- Still open: Node 22 locally against Node 24 in production and whether `runtime: 'nodejs22.x'` is
  ...
  Sanity plan (wayfinder ticket 22), which gates Studio comments.
