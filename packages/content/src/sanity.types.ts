...
// Query: {  "album": *[_type == "album" && slug.current == $slug][0]{    _id,    title,    "slug": slug.current,    date,    consentNote,    creditConfirmed,    "credit": coalesce(credit->defaultCredit, credit->name),    "edition": event->{edition, kind},    "photos": photos[]{      _key,      _type,      alt,      caption,      hotspot,      crop,      asset,      creditConfirmed,      "credit": coalesce(credit->defaultCredit, credit->name, creditNote)    }  },  "page": *[_type == "galleryPage" && _id == "galleryPage"][0]{    header{kicker{yo, en}},    creditsAndConsent,    layout{captions},    seo{title, description}  },  "settings": *[_type == "siteSettings" && _id == "siteSettings"][0]{generalEmail}}
