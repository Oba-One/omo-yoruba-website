...
- `src/queries/`: the GROQ the site runs, one file per concern (`site.ts`: `siteSettingsQuery`,
  ...
  homepage read), typed by TypeGen through `SanityQueries`.
- `src/layout.ts` (`layoutDefaults`, `withLayoutDefaults`), `src/lead-event.ts` (the season
  rule), `src/images.ts` (`createImageSet`, CDN URLs from the asset reference), `src/stega.ts`
  (the filter that keeps the discriminators and layout values clean): Phase 4, ADR 0021 and 0022.
...
- `src/routes.ts`: the public routes, which types each reads, `cacheTagsFor` (what a publish
  purges) and `tagsForRoute` (what a page carries).
