...
const meta = {
  title: 'Cards/ListRow',
  component: ListRow,
  args: { kind: 'tier', row: SPONSOR_LEVEL_PLACEHOLDERS[0] as Args['row'] },
  decorators: [wrap('oy-list')],
  parameters: {
    docs: {
      description: {
        component:
          "A row of a list. The sponsor tier form: the level's name and amount, then what it recognizes as a ticked list, each Pending while owed. The levels here are placeholders: the Studio holds none yet.",
      },
    },
  },
} satisfies Meta<Args>;
