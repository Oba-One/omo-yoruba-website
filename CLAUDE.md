...
- Sessions: `/code-review` before every commit; conventional commit messages; run
  `/handoff` before ending and save it to `docs/plans/handoff-<phase>.md`.
...
- `.claude/skills/`: this repo's skills `oy-design-system`, `oy-component`, `oy-page`,
  ...
  (user-invoked), plus the third-party `git-guardrails-claude-code` whose hook is installed
